	...
	function verifyPDFSignature(buffer)
	{
		try
		{
			...
			if(typeof acroForm === "undefined") {
				...
				document.getElementById("SigVerifyIcon").setAttribute("class","SigVerifyNo");
				document.getElementById("SigVerifyResult").innerText="  文档未签名";
			...
			if(window.isRef(fields[0]) === false) {
                document.getElementById("SigVerifyIcon").setAttribute("class","SigVerifyNo");
                document.getElementById("SigVerifyResult").innerText="  你的文档貌似坏掉了";
			...
			if("signedAttrs" in cmsSignedSimp.signerInfos[0])
			{
				...
				if(typeof crypto === "undefined") {
                    document.getElementById("SigVerifyIcon").setAttribute("class","SigVerifyNo");
                    document.getElementById("SigVerifyResult").innerText="  我貌似没办法在你的浏览器上工作";
				...
				switch(cmsSignedSimp.signerInfos[0].digestAlgorithm.algorithmId)
				{
					...
					default:
                        document.getElementById("SigVerifyIcon").setAttribute("class","SigVerifyNo");
                        document.getElementById("SigVerifyResult").innerText="  签名算法好奇怪的样子";
				...
				sequence = sequence.then((result) =>
				{
					if(result === false)
                {
                    document.getElementById("SigVerifyIcon").setAttribute("class","SigVerifyFail");
                    document.getElementById("SigVerifyResult").innerText="  小心！文档可能被篡改";
				...
				sequence = sequence.then((result) =>
				{
					...
					if(messageDigest.byteLength === 0) {
                        document.getElementById("SigVerifyIcon").setAttribute("class","SigVerifyNo");
                        document.getElementById("SigVerifyResult").innerText="  文档签名摘要缺失！";
					...
					if(view1.length !== view2.length) {
                        document.getElementById("SigVerifyIcon").setAttribute("class","SigVerifyNo");
                        document.getElementById("SigVerifyResult").innerText="  文档签名摘要错误！";
			...
			sequence = sequence.then((result) =>
			{
				if(typeof result !== "undefined")
				{
					...
					{
                        document.getElementById("SigVerifyIcon").setAttribute("class","SigVerifyFail");
                        document.getElementById("SigVerifyResult").innerText="  小心！文档疑似被篡改";
						//alert("PDF verification failed!");
						return;
					}
				}

            document.getElementById("SigVerifyIcon").setAttribute("class","SigVerifyOK");
            document.getElementById("SigVerifyResult").innerText="  恭喜！文档经过签章认证";
